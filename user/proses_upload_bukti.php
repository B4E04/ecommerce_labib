    ...
    if ($upload_ok == 1) {
        if (move_uploaded_file($_FILES["bukti_transfer"]["tmp_name"], $target_file)) {
            ...
            /* LOGIKA UNTUK ADMIN & PETUGAS:
               1. Simpan bukti pembayaran ke tabel bukti_pembayaran.
               2. Update status pesanan menjadi 'menunggu verifikasi' (agar Admin tahu ada kerjaan baru).
            */
            $path_to_save = "assets/bukti_transfer/" . $new_filename;
            
            // Insert bukti pembayaran
            $insert_bukti = mysqli_query($conn, "INSERT INTO bukti_pembayaran (transaksi_id, user_id, bukti, status) 
                                        VALUES ('$id_transaksi', '$user_id', '$path_to_save', 'menunggu')");
            
            // Update status pesanan
            $update_pesanan = mysqli_query($conn, "UPDATE pesanan SET 
                status_pesanan = 'Menunggu Verifikasi' 
                WHERE id = '$id_transaksi' AND user_id = '$user_id'");

            if ($insert_bukti && $update_pesanan) {
                echo "<script>
