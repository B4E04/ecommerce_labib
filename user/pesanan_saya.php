...
// 2. Ambil data pesanan milik user (Urutkan dari yang terbaru)
...
<div class="container">
    ...
    <?php if (mysqli_num_rows($query) > 0) : ?>
        <?php while ($row = mysqli_fetch_assoc($query)) : 
            // Logika Warna Status
            $status_raw = strtolower(str_replace(' ', '', $row['status_pesanan']));
            $status_class = 'status-pending';
            
            if (strpos($status_raw, 'menungguverifikasi') !== false) {
        ...
        ?>
            <div class="card-pesanan" style="border-left-color: <?= $border_color ?>;">
                <div class="info-left">
                    ...
                    <div class="total-price">Rp <?= number_format($row['total_bayar'], 0, ',', '.') ?></div>
                    <div class="payment-method">
                        <i class="fa-solid fa-wallet"></i> <?= strpos($row['metode_bayar'], 'VA-') === 0 ? 'Virtual Account ' . str_replace('VA-', '', $row['metode_bayar']) : strtoupper($row['metode_bayar']) ?>
                    </div>
                </div>

                <div class="action-right">
                    <div class="status-badge <?= $status_class ?>">
                        <i class="fa-solid fa-circle-dot"></i> <?= ucfirst($row['status_pesanan']) ?>
                    </div>
                    <br>
                    
                    <?php 
                    // Cek bukti pembayaran dari tabel bukti_pembayaran
                    $bukti_check = mysqli_query($conn, "SELECT bukti FROM bukti_pembayaran WHERE transaksi_id = '{$row['id']}' LIMIT 1");
                    $bukti_row = mysqli_fetch_assoc($bukti_check);
                    ?>
                    <?php if ($bukti_row && !empty($bukti_row['bukti'])) : ?>
                        <a href="../<?= $bukti_row['bukti'] ?>" target="_blank" class="btn-action btn-view">
                            <i class="fa-solid fa-image"></i> Lihat Bukti
                        </a>
                    <?php else : ?>
                        <a href="pembayaran.php?id=<?= $row['id'] ?>" class="btn-action btn-upload">
