...
if (strpos($data['metode_bayar'], 'VA-') === 0) {
    ...
    $norek = $data['nomor_va'] ?? 'VA-' . str_replace('VA-', '', $data['metode_bayar']) . '-????';
    $bank_name = str_replace('VA-', '', $data['metode_bayar']);
    $instruksi = "Buka aplikasi banking ke bank $bank_name, lalu transfer ke nomor Virtual Account di atas sesuai total yang diminta.";
} else {
    // Transfer Bank Biasa
    $rekening_bank = [
        'BCA' => ['no' => '1234567890', 'instruksi' => 'Transfer via m-BCA atau ATM BCA ke rekening di atas.'],
        'MANDIRI' => ['no' => '0987654321', 'instruksi' => 'Pilih menu Bayar/Beli > Multipayment pada aplikasi Livin\' Mandiri.'],
        'BNI' => ['no' => '1122334455', 'instruksi' => 'Transfer via ATM BNI atau aplikasi BNI Mobile Banking.']
    ];
    
    $bank = $data['metode_bayar'] ?? 'BCA';
    if (isset($rekening_bank[$bank])) {
        $norek = $rekening_bank[$bank]['no'];
        $instruksi = $rekening_bank[$bank]['instruksi'];
    } else {
        $norek = "Sistem Otomatis";
        $instruksi = "Silakan cek email atau WhatsApp Anda untuk langkah pembayaran selanjutnya.";
    }
}
?>
<!DOCTYPE html>
